...
/// See [`Config::mask_saturated_threshold`] for details.
...
/// See [`Config::mask_weight`] for details.
...
/// See [`Config::mask_weighted_counts_weight`] for details.
pub const DEFAULT_WEIGHTED_COUNTS_WEIGHT: f32 = 0.3;

/// Default weight of the chroma component of the color.
/// The more saturated the color is, the more prominent it is. This is especially true in Oklab,
/// since it accounts for low luminance affecting perceived saturation.
/// The more prominent the color is, the more likely it is to be the dominant color.
/// See [`Config::chroma_weight`] for details.
...
pub fn colors(input: InputImage) -> Vec<rgb::RGB8> {
    colors_with_config(input, Config::default()).expect("default config values should never fail")
}
...
///     chroma_weight: 0.5,
///     mask_weighted_counts_weight: 0.5,
///     ..okmain::Config::default()
/// };
/// ```
...
pub struct Config {
    ...
    ///
    /// Default value in [`DEFAULT_MASK_SATURATED_THRESHOLD`].
    pub mask_saturated_threshold: f32,
    /// The weight of the mask, which can be used to reduce the impact of the mask on
    /// less-central pixels. By default, it's set to 1.0, but by reducing this number you can
    /// increase the relative contribution of peripheral pixels.
    ///
    /// Must be in the `[0.0, 1.0]` range.
    ///
    /// Default value in [`DEFAULT_MASK_WEIGHT`].
    pub mask_weight: f32,
    /// After the number of pixels belonging to every color is added up (with the mask reducing
    /// the contribution of peripheral pixels), the sums are normalized to add up to 1.0.
    /// The sums are then used as a part of the final score that decides the ordering of the colors.
    /// This field sets the relative weight of this component in the final score.
    ///
    /// Must be in the `[0.0, 1.0]` range and add up to 1.0 together with [`Config::chroma_weight`].
    ///
    ...
    /// [`Config::mask_weighted_counts_weight`].
    ///
...
/// See [`Config`] for the tuning parameters.
pub fn colors_with_config(
    input: InputImage,
    config: Config,
) -> Result<Vec<rgb::RGB8>, ConfigError> {
    colors_debug(input, config).map(|(colors, _)| colors)
}
...
fn colors_internal(
    ...
    config: Config,
        ...
        (0.0..0.5).contains(&config.mask_saturated_threshold),
        InvalidMaskSaturatedThresholdSnafu {
            threshold: config.mask_saturated_threshold
        }
    );
    ensure!(
        (0.0..=1.0).contains(&config.mask_weight),
        InvalidMaskWeightSnafu {
            weight: config.mask_weight
        }
    );
    ensure!(
        (0.0..=1.0).contains(&config.mask_weighted_counts_weight),
        InvalidWeightedCountsWeightSnafu {
            weight: config.mask_weighted_counts_weight
        }
    );
    ensure!(
        (0.0..=1.0).contains(&config.chroma_weight),
        InvalidChromaWeightSnafu {
            weight: config.chroma_weight
        }
    );
    let weight_sum = config.mask_weighted_counts_weight + config.chroma_weight;
    ensure!(
        ...
        WeightsDontAddUpSnafu {
            mask_weighted_counts_weight: config.mask_weighted_counts_weight,
    ...
    for (i, &assignment) in centroids_result.assignments.iter().enumerate() {
        ...
        let mask_value = distance_mask(
            config.mask_saturated_threshold,
            oklab_soa.width,
            oklab_soa.height,
            x,
            y,
        ...
        let w = 1.0 - config.mask_weight * (1.0 - mask_value);
    ...
    let mut scored_centroids = centroids_result
        ...
        .map(|(i, oklab)| {
            ...
            let mask_weighted_counts_score =
                mask_weighted_counts * config.mask_weighted_counts_weight;
            ...
            let chroma_score = chroma * config.chroma_weight;
